...
#
# NOTICE: Verbatim port of unitree_rl_lab ``unitree_rl_lab/utils/parser_cfg.py``
# (the upstream file carries no per-file license header; the upstream
# repository is licensed under Apache-2.0). It generalizes Isaac Lab's
# ``parse_env_cfg`` with an ``entry_point_key`` argument so play/eval scripts
...
def parse_env_cfg(
    ...
    Args:
        ...
        entry_point_key: The gym-registration kwargs key holding the config class
            (e.g. ``"play_env_cfg_entry_point"``).
